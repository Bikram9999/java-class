package ThapaTakeOutProject;

import java.util.Scanner;

public class ThapaTakeOut {
    ...
    public static void main(String[] args) {
      ...
      do {
          ...
          switch (mealType) {
              ...
              case "1":
                  ThapaTakeOut.displayItalianFoodMenu();
                  int[] feedItalianFood = {5, 7, 4};
                  double[] priceItalian = {17.99, 15.99, 12.99};
                  int opt = ThapaTakeOut.foodChoice(feedItalianFood, priceItalian);
                  ThapaTakeOut.finalPrice(feedItalianFood, priceItalian, opt, numberOfPeople);
                  isWrongMenuChoice = false;
                  break;

              case "2":
                  ThapaTakeOut.displayChineseFoodMenu();
                  int[] feedChineseFood = {7, 7, 5};
                  double[] priceChinese = {18.99, 18.99, 10.99};
                  opt = ThapaTakeOut.foodChoice(feedChineseFood, priceChinese);
                  ThapaTakeOut.finalPrice(feedChineseFood, priceChinese, opt, numberOfPeople);
                  isWrongMenuChoice = false;
                  break;

              case "3":
                  ThapaTakeOut.displayAmericanFoodMenu();
                  int[] feedAmericanFood = {8, 5, 10};
                  double[] priceAmerican = {21.99, 22.99, 26.99};
                  opt = ThapaTakeOut.foodChoice(feedAmericanFood, priceAmerican);
                  ThapaTakeOut.finalPrice(feedAmericanFood, priceAmerican, opt, numberOfPeople);
                  isWrongMenuChoice = false;
                  break;
              default:
  ...
  public static void finalPrice(int[] feeds, double[] prices, int choice, int guestCount) {
      int trays = ThapaTakeOut.determineTrays(guestCount, feeds[choice-1]);
      double subTotal = ThapaTakeOut.getSubtotal(prices[choice - 1], trays);
      double tax = ThapaTakeOut.getTax(subTotal, .07);
      double tip = ThapaTakeOut.getTip(subTotal, .15);
      double grandTotal = ThapaTakeOut.getGrandTotal(subTotal, tax, tip);
      ...
      System.out.printf("Price per person: $ %.2f\n", ThapaTakeOut.pricePerPerson(grandTotal, guestCount));
      System.out.println("Left-over servings for the delivery person: " + ThapaTakeOut.determineLeftOvers(feeds[choice - 1], trays, guestCount));
